...
<form action="{{ route('users.store') }}" class="form1" method="post">
    ...
    <h3>PW <input type="password" name="pw" minlength="5" maxlength="10" pattern="^(?=.*[a-zA-Z])(?=.*[0-9])[a-zA-Z0-9]{10,20}$" placeholder="英数字を含む文字数10～20文字以内" class="text5" required></h3>
    <h3>PW確認 <input type="password" name="pwasr" minlength="5" maxlength="10" pattern="^(?=.*[a-zA-Z])(?=.*[0-9])[a-zA-Z0-9]{10,20}$" placeholder="英数字を含む文字数10～20文字以内" class="text6" required></h3>
